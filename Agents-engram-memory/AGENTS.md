<!-- ENGRAM-MEMORY-START (crisol-definitive) -->
...
## 🧠 Engram MCP Tools — Referencia Rápida

Tengo acceso a Engram v1.20.0 como MCP server local.

### Memorias (CRUD)
| Tool | Para qué |
|---|---|
| `engram_mem_save(...)` | Guardar decisión, bugfix, descubrimiento, patrón |
| `engram_mem_search(query)` | Buscar memorias por texto completo (FTS5) |
| `engram_mem_get_observation(id)` | Contenido completo sin truncar |
| `engram_mem_update(id, ...)` | Corregir o actualizar una memoria existente |
| `engram_mem_delete(id)` | Eliminar memoria (soft-delete por defecto, hard opcional) |
| `engram_mem_suggest_topic_key(...)` | Obtener topic_key estable para upserts |
| `engram_mem_context(project?)` | Contexto rápido de sesiones recientes |
| `engram_mem_timeline(observation_id)` | Contexto cronológico alrededor de una observación |
| `engram_mem_current_project()` | Detectar proyecto activo desde el directorio |
| `engram_mem_pin(id)` | Fijar memoria al inicio del contexto |
| `engram_mem_unpin(id)` | Sacar memoria del inicio del contexto |
| `engram_mem_review(action)` | Listar observaciones vencidas o marcar como revisadas |
| `engram_mem_merge_projects(from, to)` | Fusionar variantes de nombre de proyecto |

### Sesiones
| Tool | Para qué |
|---|---|
| `engram_mem_session_start(id)` | Registrar inicio de sesión |
| `engram_mem_session_end(id)` | Cerrar sesión |
| `engram_mem_session_summary(content)` | Resumen estructurado de cierre |

### Prompts y Juicio
| Tool | Para qué |
|---|---|
| `engram_mem_save_prompt(content)` | Guardar prompt del usuario para contexto |
| `engram_mem_judge(judgment_id, relation)` | Resolver conflictos de memoria |
| `engram_mem_compare(id_a, id_b, relation)` | Comparar dos memorias y registrar relación |
| `engram_mem_doctor(check?)` | Diagnósticos del estado de Engram |
| `engram_mem_capture_passive(content)` | Extraer aprendizajes de texto al finalizar tarea |

### CLI (terminal)
| Comando | Para qué |
|---|---|
| `engram projects list` | Ver proyectos con conteo de observaciones |
| `engram search "query"` | Búsqueda rápida desde terminal |
| `engram tui` | Explorador visual de memorias |
| `engram sync` | Exportar a `.engram/` para git tracking |
| `engram sync --import` | Importar en otra máquina |
<!-- ENGRAM-MEMORY-END -->
