/** Read-only validation of metadata, preservation and byte-level integrity. Scripts are never executed. */
import { tool } from "@opencode-ai/plugin"
import { createHash } from "node:crypto"
import { readFile, readdir, stat } from "node:fs/promises"
import path from "node:path"

const RESTART = "La validación terminó. Si la skill fue instalada o actualizada, no está disponible en la sesión actual: el usuario humano debe reiniciar completamente el runtime/servidor de OpenCode antes de usarla."
const OPS = new Set(["reviewed", "adapted", "unknown"]); const ROUTING = new Set(["refiner-only", "refiner-north", "executor-auditor", "transversal"]); const REASONING = new Set(["none", "procedural", "diagnostic", "architectural", "creative"]); const AGENTS = new Set(["refiner", "north", "executor", "auditor"])
...
async function filesOnly(dir: string, prefix = ""): Promise<string[]> { const out: string[] = []; for (const e of await readdir(dir, { withFileTypes: true })) { const rel = prefix ? `${prefix}/${e.name}` : e.name; const full = path.join(dir, e.name); if (e.isDirectory()) out.push(...await filesOnly(full, rel)); else if (e.isFile() && safe(rel)) out.push(rel) } return out }
